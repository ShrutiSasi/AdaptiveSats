...
Naive DCA invests a fixed amount at regular intervals, but may be inefficient because it does not adjust to changing Bitcoin market conditions.
...
Long-term Bitcoin accumulation is a practical problem for institutions and foundations that want to accumulate Bitcoin over time. One common approach is Dollar-Cost Averaging (DCA), where a fixed amount is invested regularly. We use naive DCA as the baseline, but it has limitations because it does not adjust to market conditions. For institutions with a fixed accumulation budget, this may reduce accumulation efficiency because the strategy does not allocate more capital during favorable buying periods. This problem is important because many smaller institutions do not have the same information advantages or market access as larger financial institutions. Bitcoin provides a transparent setting because its market and on-chain data are public and auditable, enabling data-driven accumulation strategies. We evaluate whether public Bitcoin market and on-chain data can support an adaptive DCA strategy that improves Sats per Dollar compared with naive DCA, while using the same fixed accumulation budget. For this analysis, we first perform exploratory data analysis, apply dimensionality reduction techniques, develop and evaluate adaptive allocation strategies against naive DCA through backtesting.
