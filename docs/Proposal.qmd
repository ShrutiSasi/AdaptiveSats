...
The approach develops an adaptive DCA strategy that adjusts allocations using public signals. The goal is not to predict short-term prices, but to improve accumulation efficiency by acquiring more Bitcoin for the same fixed budget. We measure this using Sats per Dollar, which measures how much Bitcoin is accumulated for each dollar invested. For this, we use the Bitcoin Research Kit (BRK) metrics dataset from the StackSats ecosystem, which contains daily market and on-chain metrics from year 2009 to 2026. Because the dataset includes more than 41,000 metric keys, we perform exploratory data analysis and apply feature reduction and dimensionality reduction techniques to manage the high-dimensional feature space. We then develop and backtest adaptive allocation strategies against a naive DCA baseline.
The final product will be a reproducible framework that will help the Trilemma Foundation and open-source community assess adaptive Bitcoin accumulation strategies against naive DCA.


# Introduction

Long-term Bitcoin accumulation is a practical problem for institutions and foundations that want to accumulate Bitcoin over time. One approach is Dollar-Cost Averaging (DCA), where the same dollar amount is invested at regular intervals regardless of market conditions. We use naive DCA as the baseline strategy but this creates a limitation. For institutions with a fixed accumulation budget, this may reduce accumulation efficiency because the strategy does not allocate more capital during favorable buying periods. This problem is important because many smaller institutions do not have the same information advantages or market access as larger financial institutions. Bitcoin offers a transparent setting because its market and on-chain data are public and auditable. This creates an opportunity to build data-driven accumulation strategies using public information. We evaluate whether public Bitcoin market and on-chain data can support an adaptive DCA strategy that improves Sats per Dollar compared with naive DCA, while using the same fixed accumulation budget. For this analysis, we first perform exploratory data analysis and apply dimensionality reduction techniques. We then develop and evaluate adaptive allocation strategies against naive DCA through backtesting.
The final product will include a cleaned data pipeline, strategy and backtesting logic, and evaluation results comparing adaptive accumulation against naive DCA for the Trilemma Foundation and the open-source community.
