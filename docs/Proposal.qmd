...
This project evaluates whether public Bitcoin (@nakamoto2008) market and on-chain data can improve long-term Bitcoin accumulation compared with naive Dollar-Cost Averaging (DCA), which serves as our primary baseline strategy.
Naive DCA invests the same dollar amount at regular intervals regardless of price or market conditions. While it is simple, it may be inefficient because it allocates the same amount during periods when Bitcoin is potentially overvalued, undervalued, or under market stress.
The approach develops an adaptive DCA strategy that adjusts allocations using public signals. The goal is not to predict short-term prices, but to improve accumulation efficiency by acquiring more Bitcoin for the same fixed budget. We measure this using Sats per Dollar, which measures how much Bitcoin is accumulated for each dollar invested.
For this, we use the BRK Bitcoin metrics dataset from the StackSats ecosystem, which contains daily market and on-chain metrics from 2009 to 2026. Because the dataset includes more than 41,000 metric keys, we perform exploratory data analysis and look to apply feature reduction and dimensionality reduction techniques to manage the high-dimensional feature space. We then develop and backtest adaptive allocation strategies against a naive DCA baseline.
The expected outcome is a reproducible accumulation strategy framework for the Trilemma Foundation and the open-source community. The final product will include a cleaned data pipeline, strategy and backtesting logic, evaluation results, and visual outputs comparing adaptive accumulation against naive DCA.


# Introduction

Long-term Bitcoin accumulation is a practical problem for institutions and foundations that want to accumulate Bitcoin over time. One approach is Dollar-Cost Averaging (DCA), where the same dollar amount is invested at regular intervals regardless of price. We use naive DCA as the baseline strategy, where the same amount is invested at each interval without adjusting for market conditions. DCA is simple and easy to implement, but it does not respond to changing market conditions.
This creates a limitation. Naive DCA buys the same amount when Bitcoin is potentially overvalued, undervalued, or under market stress. For institutions with a fixed accumulation budget, this may reduce accumulation efficiency because the strategy does not allocate more capital during favorable buying periods.
This problem is important because many smaller institutions do not have the same information advantages or market access as larger financial institutions. Bitcoin offers a transparent setting because its market and on-chain data are public and auditable. This creates an opportunity to build data-driven accumulation strategies using public information.
We evaluate whether public Bitcoin market and on-chain data can support an adaptive DCA strategy that achieves higher Sats per Dollar than naive DCA. Here, Sats per Dollar measures how much Bitcoin is accumulated for each dollar invested. The goal is not to predict short-term prices, but to improve long-term accumulation efficiency using the same fixed budget.
For this, we use the Bitcoin Research Kit (BRK) Bitcoin metrics dataset from the StackSats ecosystem, which contains daily Bitcoin market and on-chain metrics from 2009 to 2026. Since the dataset contains more than 41,000 metric keys, we first perform exploratory data analysis and look to apply feature reduction and dimensionality reduction techniques to manage the high-dimensional feature space. We then develop and evaluate adaptive allocation strategies against naive DCA through backtesting.
The final product will be a reproducible accumulation strategy framework that includes a cleaned data pipeline, strategy and backtesting logic, evaluation results, and visual outputs comparing adaptive accumulation against naive DCA. This product is intended to help the Trilemma Foundation and the open-source community evaluate whether public Bitcoin data can support smarter long-term accumulation strategies.
