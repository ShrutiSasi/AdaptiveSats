...
Traditional financial institutions rely on sophisticated execution systems, proprietary data pipelines, and privileged market access to optimize investment decisions, while smaller institutions and retail participants operate with more limited resources. Bitcoin partially reduces this informational gap because all transaction activity is recorded publicly on-chain, enabling open analysis of network behaviour, market structure, and investor activity.

This transparency creates an opportunity to investigate whether publicly available blockchain data can improve long-term Bitcoin accumulation strategies. In particular, we examine whether on-chain signals can identify periods of relative undervaluation and improve capital allocation compared to traditional fixed-schedule investing approaches.

The most common strategy, naive Dollar-Cost Averaging (DCA), invests a fixed amount at regular intervals regardless of market conditions. While simple and robust, it does not account for changing valuation dynamics, investor sentiment, or network behaviour.

# Introduction

We use the BRK on-chain dataset, containing over 41,000 metrics spanning market valuation, network activity, holder behaviour, technical indicators, and macro-level blockchain statistics. Due to high dimensionality and feature redundancy, we apply a structured feature selection pipeline using feature grouping, hierarchical clustering, and correlation-based pruning to identify informative signals.

Using this reduced feature space, we design adaptive accumulation strategies that adjust daily allocation weights under realistic constraints. Approaches include Value Averaging, moving-average trend allocation, and signal-driven models based on historical on-chain conditions. Performance is evaluated against naive DCA using rolling-window backtesting and Sats per Dollar (SPD), which measures Bitcoin accumulated per unit of USD invested.

Ultimately, the project investigates whether publicly available blockchain data can systematically improve Bitcoin accumulation efficiency without relying on proprietary information or high-frequency trading infrastructure.
